...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate a trained CustomDiT checkpoint by decoding only the generated latent stream "
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a CustomDiT checkpoint. Overrides --resume-dir when both are provided.",
    )
    parser.add_argument(
        "--resume-dir",
        type=str,
        default=None,
        help="CustomDiT training output directory containing checkpoint-last.pth.",
    )
    parser.add_argument(
        "--checkpoint-key",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to torch-fidelity FID stats. Defaults to custom/fid_stats/custom_in{image_size}_stats.npz.",
    )
    parser.add_argument(
        "--vae-pretrained-path",
        type=str,
    ...
    parser.add_argument("--sampling-atol", type=float, default=None)
    parser.add_argument("--sampling-rtol", type=float, default=None)
...
def load_checkpoint_args(checkpoint_payload: dict) -> argparse.Namespace:
    ...
    raise KeyError(
        "CustomDiT checkpoint does not contain a supported `args` payload. "
...
def select_checkpoint_key(args: argparse.Namespace, checkpoint_payload: dict) -> str:
    ...
    if args.checkpoint_key not in checkpoint_payload:
        raise KeyError(
            f"Checkpoint key `{args.checkpoint_key}` not found in the CustomDiT checkpoint."
        )
...
def apply_generation_overrides(
    ...
    overrides = {
        ...
        "sampling_atol": cli_args.sampling_atol,
        "sampling_rtol": cli_args.sampling_rtol,
        "timestep_shift": cli_args.timestep_shift,
...
def resolve_denoiser_state_dict(
    ...
    if not isinstance(checkpoint_state, dict):
        raise TypeError(
            "CustomDiT checkpoint entry must be a state dict mapping, "
    ...
    raise RuntimeError("CustomDiT checkpoint state dict is incompatible with the evaluation model.")
...
def main() -> None:
    ...
    try:
        ...
        if not checkpoint_path.is_file():
            raise FileNotFoundError(f"CustomDiT checkpoint not found: {checkpoint_path}")
        ...
        log_rank0(rank, f"Loading CustomDiT checkpoint from {checkpoint_path}")
        ...
        if args.fid_stats_path is None:
            raise FileNotFoundError(
                "No FID statistics file was provided and no built-in CustomDiT stats file matches "
        ...
        log_rank0(rank, f"Decoding CustomDiT latents with VAE `{args.vae_pretrained_path}`")
        ...
        if rank == 0:
            ...
            summary = {
                ...
                "sampling_atol": float(model.sampling_atol),
                "sampling_rtol": float(model.sampling_rtol),
                "class_num": class_num,
