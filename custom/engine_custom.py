...
import custom.util.lr_sched as lr_sched
from custom.eval.diffusion_decoder import decode_with_decoder
...
def train_one_epoch(
    ...
    for optimizer_step, (micro_batches, micro_batches_in_update) in enumerate(
        ...
        # Per optimizer step (instead of per micro-batch) lr scheduling keeps
        # accumulation aligned with the intended large-batch training recipe.
...
def evaluate(model_without_ddp, args, epoch, decoder, batch_size=64, log_writer=None, wandb_run=None, wandb_step=None):
    ...
    save_folder = os.path.join(
        args.output_dir,
        "{}-steps{}-cfg{}-interval{}-{}-shift{}-image{}-res{}".format(
            ...
            model_without_ddp.timestep_shift,
