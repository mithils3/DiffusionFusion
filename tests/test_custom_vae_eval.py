...
from custom.eval import vae_eval as vae_eval_module
...
class VaeEvalHelperTests(unittest.TestCase):
    ...
    def test_load_checkpoint_args_accepts_dict_payload(self):
        loaded_args = vae_eval_module.load_checkpoint_args(
            {"args": {"model": "CustomDiT-B/2-4C", "cfg": 2.9}}
        )
    ...
    def test_apply_generation_overrides_updates_sampling_fields(self):
        ...
        self.assertEqual(eval_args.sampling_atol, 1e-5)
        self.assertEqual(eval_args.sampling_rtol, 1e-3)
        self.assertEqual(eval_args.timestep_shift, 0.3)
