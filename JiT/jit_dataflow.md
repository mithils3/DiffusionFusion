...
flowchart LR
  ...
  DROP --> FINAL["FinalLayer (AdaLN + Linear)<br/>prediction patches [B,N,p*p*C] e.g. [8,256,768]"]
...
- `FinalLayer`: final AdaLN + linear projection from token features to prediction patches.
- `unpatchify`: reshapes patch outputs back to full latent/DINO prediction tensors.
- `x_pred`: predicted denoised state at the current step.
